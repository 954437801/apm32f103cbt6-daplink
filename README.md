================================================================================
  APM32F103CBT6 DAPLink 96MHz 适配 + IAP冷启动丢失修复 总结
================================================================================

一、问题现象
-----------
  1. BL(72MHz) + IF(96MHz) 通过 IAP 拖入 IF CRC hex 后热启动正常
  2. USB拔插(冷启动)后回退到 MAINTENANCE 模式
  3. 直接 SWD 刷 IF hex 则正常保持 DAPLink

二、根因分析
-----------
  1. BL 中 flash_regions[0].end 设为 0x08020000（整片128KB结尾）
...
---------------------
  ...
  文件1: source/board/stm32f103xb_bl.c
  ────────────────────────────────────
  - 添加 #include "daplink_addr.h"
  - flash_regions[0].end:
    修改前: 0x08000000 + KB(128)          = 0x08020000  (擦80页)
    修改后: DAPLINK_ROM_IF_START +
            DAPLINK_ROM_IF_SIZE           = 0x0801FC00  (擦79页)
    效果: IAP 不再擦除配置区，配置区保留原始数据

  文件2: source/hic_hal/stm32/stm32f103xb/sdk.c
  ──────────────────────────────────────────────
  - 添加 FLASH_LATENCY_3 宏定义（APM32 SDK 无此宏）
  - 差异化时钟策略：
    BL: PLL_MUL9(72MHz) + FLASH_LATENCY_2  → IAP兼容性好
...
五、验证结果
-----------
  ✓ BL 编译: 48KB（100% 填满，完美适配）
  ✓ IF 编译: 79KB + CRC4B
  ✓ SWD 烧录 BL → 进入 MAINTENANCE
  ✓ SWD 烧录 IF → DAPLink 正常
  ✓ USB 拔插后 → 仍然保持 DAPLink
  ✓ IAP 拖入 IF CRC hex → 正常工作
